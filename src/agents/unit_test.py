"""
Agente: UnitTest (Generador y Ejecutor de Unit Tests)
...
def tester_merge_node(state: AgentState) -> AgentState:
    with agent_execution_context("🐙 TESTER - SQUASH MERGE", logger):
        log_agent_execution(logger, "UnitTest", "merge_iniciado", {
        ...
        if merged:
            logger.info(f"✅ PR #{pr_number} mergeada (squash) por UnitTest")
...
def unit_test_node(state: AgentState) -> AgentState:
    """
    Nodo de UnitTest - Genera y ejecuta tests unitarios.
    ...
    """
    with agent_execution_context("🧪 UNIT TEST", logger):
        log_agent_execution(logger, "UnitTest", "iniciado", {
            "requisito_id": state['attempt_count'],
            "debug_attempt": state['debug_attempt_count']
        })
        
        # === INICIO: Actualizar estado del Work Item de UnitTest a "In Progress" ===
        if (settings.AZURE_DEVOPS_ENABLED and 
            ...
            try:
                ...
                logger.info(f"🔄 Actualizando estado de Task de UnitTest #{task_id} a 'In Progress'...")
        ...
        - `src/test/{test_filename}` - Tests unitarios (por UnitTest)
