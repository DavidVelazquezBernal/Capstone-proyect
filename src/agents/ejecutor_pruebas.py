...
def ejecutor_pruebas_node(state: AgentState) -> AgentState:
    ...
    """
    print(f"\n--- 4 🧪 Ejecutor de Pruebas --- Ejecutar tests unitarios")
    ...
    if not os.path.exists(test_path):
        print(f"   ❌ ERROR: No se encontró el archivo de tests: {test_path}")
    ...
    if not os.path.exists(code_path):
        print(f"   ⚠️ AVISO: No se encontró el archivo de código: {code_path}")
        ...
        print(f"   ✅ Código guardado desde el estado")
    
    print(f"   📄 Archivo de tests: {test_filename}")
    print(f"   📄 Archivo de código: {code_filename}")
    ...
    try:
        ...
        if result['success']:
            ...
            if isinstance(stats, dict):
                ...
                print(f"   ✅ Tests PASSED - Total: {total}, Pasados: {passed}, Fallidos: {failed}")
            else:
            ...
            guardar_fichero_texto(
                f"4_probador_req{attempt}_debug{debug_attempt}_PASSED.txt",
                output_content,
                directorio=settings.OUTPUT_DIR
            )
        else:
            ...
            if isinstance(stats, dict):
                ...
                passed = stats.get('passed', 0)
                failed = stats.get('failed', 0)
                print(f"   ❌ Tests FAILED - Total: {total}, Pasados: {passed}, Fallidos: {failed}")
            else:
            ...
            guardar_fichero_texto(
                f"4_probador_req{attempt}_debug{debug_attempt}_FAILED.txt",
                output_content,
                directorio=settings.OUTPUT_DIR
            )
...
def _ejecutar_tests_typescript(test_path: str, code_path: str, state: AgentState) -> Dict[str, Any]:
    ...
    try:
        ...
        if not os.path.exists(package_json_path):
            ...
            print(f"   ℹ️  package.json creado en {output_dir}")
...
def _mostrar_resumen_ejecucion(result: Dict[str, Any]) -> None:
    ...
    print("-" * 60)
    print(f"Estado: {'✅ PASSED' if result['success'] else '❌ FAILED'}")
    ...
    if isinstance(stats, dict):
        ...
        print(f"Tests ejecutados: {stats}")
